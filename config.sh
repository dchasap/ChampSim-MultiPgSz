...
# TODO: we should parametrize page sizes 
vmem_multi_pgsz_fmtstr = 'VirtualMemory vmem({attrs[size]}, 1 << 12, 1 << 21, {attrs[num_levels]}, 1, {attrs[minor_fault_penalty]});\n'
...
default_stlb = { 'sets': 128, 'ways': 12, 'rq_size': 32, 'wq_size': 32, 'pq_size': 0, 'mshr_size': 16, 'latency': 8, 'fill_latency': 1, 'max_read': 1, 'max_write': 1, 'prefetch_as_load': False, 'virtual_prefetch': False, 'wq_check_full_addr': False, 'prefetch_activate': 'LOAD,PREFETCH', 'prefetcher': 'no', 'replacement': 'dpi'}
...
with open(instantiation_file_name, 'wt') as wfp:
    ...
    if 'multiple_page_size' in config_file.keys():
      wfp.write(vmem_multi_pgsz_fmtstr.format(attrs=config_file['virtual_memory']))
    else:
...
with open(constants_header_name, 'wt') as wfp:
    ...
    if 'multiple_page_size' in config_file.keys():
      wfp.write('#define MULTIPLE_PAGE_SIZE\n')
      wfp.write(define_fmtstr.format(name='base_page_size').format(names=const_names, config=config_file))
      wfp.write(define_log_fmtstr.format(name='base_page_size').format(names=const_names, config=config_file))
      wfp.write(define_fmtstr.format(name='large_page_size').format(names=const_names, config=config_file))
      wfp.write(define_log_fmtstr.format(name='large_page_size').format(names=const_names, config=config_file))
...
with open('Makefile', 'wt') as wfp:
    ...
    wfp.write('CFLAGS := ' + config_file.get('CFLAGS', '-g -Wall -O3') + ' -std=gnu99\n')
    wfp.write('CXXFLAGS := ' + config_file.get('CXXFLAGS', '-g -Wall -O3') + ' -std=c++17\n')
