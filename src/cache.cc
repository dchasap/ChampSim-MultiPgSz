  ...
  while (writes_available_this_cycle > 0) {
    ...
    if (fill_mshr == std::end(MSHR) || fill_mshr->event_cycle > current_cycle)
      return;
		...
		// FIXME: Need to find a way to differenciate between TLBs and other caches
		// find victim
		uint32_t set;
		if (fill_mshr->page_size == LARGE_PAGE_SIZE && false) {
			set = get_set(fill_mshr->base_vpn);
		}
		else {
...
#ifdef MULTIPLE_PAGE_SIZE 
		std::cout << " page_size: " << handle_pkt.page_size;
#else
		std::cout << " page_size: NA";
#endif
		std::cout << std::endl;	
  });
...
#ifdef MULTIPLE_PAGE_SIZE
			writeback_packet.page_size = handle_pkt.page_size;
...
#ifdef MULTIPLE_PAGE_SIZE
      std::cout << " address: " << std::hex << (entry.address >> lg2(entry.page_size)) << " full_addr: " << entry.address << std::dec << " type: " << +entry.type;
#else
