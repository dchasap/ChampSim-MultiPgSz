...
void print_tlb_stats() {
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
		std::cout << " TOTAL_BASE_PAGES: " << std::setw(10) << ooo_cpu[i]->total_base_pages  << std::endl;
		std::cout << " TOTAL_LARGE_PAGES: " << std::setw(10) << ooo_cpu[i]->total_large_pages  << std::endl << std::endl;
	}
}
#endif
...
int main(int argc, char** argv)
{
  ...
  std::cout << std::endl;
	...
	std::cout << "VirtualMemory large page size: " << LARGE_PAGE_SIZE << " base log2_page_size: " << LOG2_LARGE_PAGE_SIZE << std::endl;
#else
  ...
  for (int i = optind; i < argc; i+=2) {
    std::cout << "CPU " << traces.size() << " runs " << argv[i] << std::endl;
		...
		traces.push_back(get_tracereader(argv[i], argv[i+1], traces.size(), knob_cloudsuite));
#else
