...
void O3_CPU::fetch_instruction()
{
  ...
  // fetch cache lines that were part of a translated page but not the cache
	...
	// MULTIPLE_PAGE_SIZE: We allocate all instructions to 4KB pages
	auto l1i_req_begin =
...
{
	// TODO: I don't undestand this translation to pa
...
//	std::cout << "instr_id:" << lq_it->instr_id << ", page_size:" << lq_it->page_size << std::endl;
//	std::cout << "LARGE_PAGE_SIZE:" << LARGE_PAGE_SIZE << std::endl;
//	std::cout << "BASE_PAGE_SIZE:" << BASE_PAGE_SIZE << std::endl;
	lq_it->base_vpn = rob_it->source_base_vpn[data_index];
#endif
  ...
  while (available_fetch_bandwidth > 0 && to_read > 0 && !ITLB_bus.PROCESSED.empty()) {
    ...
    while (available_fetch_bandwidth > 0 && !itlb_entry.instr_depend_on_me.empty()) {
      auto it = itlb_entry.instr_depend_on_me.front();
			//MULTIPLE_PAGE_SIZE: Instructions are allocated on 4KB pages
        ...
        // LOG2_PAGE_SIZE) && it->translated != 0)
        {
          ...
          // translated physical page address
					//MULTIPLE_PAGE_SIZE: Instructions are allocated on 4KB pages
          it->instruction_pa = splice_bits(itlb_entry.data, it->ip, LOG2_PAGE_SIZE); // this basically concats ppage_num + offset 
				}
...
#ifdef MULTIPLE_PAGE_SIZE
			// all entries should have been on the same page, so page size is the same
			assert(dtlb_entry.page_size == sq_merged->page_size);
      sq_merged->physical_address = splice_bits(dtlb_entry.data, sq_merged->virtual_address,
                                                lg2(dtlb_entry.page_size)); // translated address
/*
			std::cout << "            dtlb_entry.data: " << std::hex << dtlb_entry.data << std::endl;
			std::cout << " sq_merged->virtual_address: " << std::hex << sq_merged->virtual_address << std::endl;
			std::cout << "sq_merged->physical_address: " << std::hex << sq_merged->physical_address << std::dec << std::endl;
			std::cout << "       dtlb_entry.page_size: " << dtlb_entry.page_size << std::endl;
			std::cout << "       sq_merged->page_size: " << sq_merged->page_size << std::endl;
*/
#else
			sq_merged->physical_address = splice_bits(dtlb_entry.data, sq_merged->virtual_address,
...
#ifdef MULTIPLE_PAGE_SIZE
			// same, all entries are on the same page
			assert(dtlb_entry.page_size == lq_merged->page_size);
      lq_merged->physical_address = splice_bits(dtlb_entry.data, lq_merged->virtual_address,
                                                lg2(dtlb_entry.page_size)); // translated address
/*
			std::cout << "            dtlb_entry.data: " << std::hex << dtlb_entry.data << std::endl;
			std::cout << " lq_merged->virtual_address: " << std::hex << lq_merged->virtual_address << std::endl;
			std::cout << "lq_merged->physical_address: " << std::hex << lq_merged->physical_address << std::dec << std::endl;
			std::cout << "       dtlb_entry.page_size: " << dtlb_entry.page_size << std::endl;
			std::cout << "       lq_merged->page_size: " << lq_merged->page_size << std::endl;
*/
#else
      lq_merged->physical_address = splice_bits(dtlb_entry.data, lq_merged->virtual_address,
                                                LOG2_PAGE_SIZE); // translated address
